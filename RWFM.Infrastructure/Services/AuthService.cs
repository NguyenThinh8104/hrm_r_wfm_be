...
public class AuthService : IAuthService
{
    ...
    public async Task<ApiResponse<AuthResponseDto>> KioskLoginAsync(KioskLoginRequestDto request)
    {
        ...
        // Kiểm tra xem nhân viên có thuộc chi nhánh này hoặc đang có lệnh điều động hợp lệ đến chi nhánh này không
        ...
        var user = employee.User ?? new User
        {
            ...
            Role = employee.Position?.PositionCode ?? "STAFF",
            IsActive = true
        };
    ...
    public async Task<ApiResponse<List<UserSummaryDto>>> GetStoreEmployeesAsync(int storeId)
    {
        ...
        var result = employees.Select(e => MapUserSummary(e.User ?? new User { UserId = e.UserId ?? 0, Username = e.EmployeeCode, Role = e.Position?.PositionCode ?? "STAFF" }, e)).ToList();
    ...
    {
        "CHAIN_ADMIN" => "Quản trị Chuỗi (Admin)",
        "STORE_MANAGER" => "Cửa hàng trưởng (Store Manager)",
        "SHIFT_LEADER" => "Trưởng ca (Shift Leader)",
        "CASHIER" => "Nhân viên Thu ngân (Cashier)",
        "SALES_STAFF" => "Nhân viên Bán hàng (Sales Staff)",
        "SECURITY_GUARD" => "Nhân viên Bảo vệ (Security Guard)",
        _ => role
    };
}
